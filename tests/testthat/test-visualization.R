...
# Helper: Create sait_res dataframe with adj_p_interaction format
create_sait_res_adj_p_interaction <- function() {
  data.frame(
...
# Helper: Create sait_res dataframe with p_interaction format
create_sait_res_p_interaction <- function() {
  data.frame(
    gene = c("GENE_1", "GENE_2", "GENE_3", "GENE_4", "GENE_5"),
    coef = c(-0.5, 0.3, -0.2, 0.6, 0.1),
...
# Helper: Create sait_res dataframe with p_value format
create_sait_res_p_value <- function() {
  data.frame(
    gene = c("GENE_1", "GENE_2", "GENE_3", "GENE_4", "GENE_5"),
    statistic = c(10.5, 12.3, 8.7, 15.2, 6.1),
...
# ==============================================================================
# TEST: gene vs sait_res resolution (Lines 570-571)
# ==============================================================================
...
test_that("plot_diversity_spectrum gene-mode: gene parameter takes precedence (line 570)", {
  ...
  # When gene is provided, sait_res should be ignored
  result <- suppressWarnings(tryCatch({
    plot_diversity_spectrum(
      analysis,
      gene = "GENE_2",  # gene is provided
      sait_res = sait_res,  # sait_res also provided
      assay_name = "diversity"
    )
  }, error = function(e) list(error = conditionMessage(e))))
...
# ==============================================================================
# TEST: sait_res validation (Line 572)
# ==============================================================================
...
test_that("plot_diversity_spectrum gene-mode: sait_res must be data.frame (line 572)", {
  ...
  suppressWarnings(expect_error(
    plot_diversity_spectrum(
      analysis,
      gene = NULL,
      sait_res = list(not_a_dataframe = TRUE),  # Not a data.frame
      assay_name = "diversity"
    ),
    "Missing sample_type mapping"
  ))
...
test_that("plot_diversity_spectrum gene-mode: n_top limits gene selection (line 593)", {
  ...
  # n_top should limit selection (5 genes in sait_res, pick top 2)
  result <- suppressWarnings(tryCatch({
    plot_diversity_spectrum(
      analysis,
      gene = NULL,
...
test_that("plot_divergence_spectrum: top N genes selection from sait_res", {
  ...
  n_genes <- 3
  sait_sorted <- sait_res[order(sait_res$adj_p_interaction, na.last = TRUE), ]
  top_genes <- head(sait_sorted$gene, n_genes)
...
test_that("plot_divergence_spectrum: gene column detection in sait_res", {
  ...
  sait_res_gene <- data.frame(
    gene = c("g1", "g2", "g3"),
    adj_p_interaction = c(0.001, 0.01, 0.1)
  )
  
  gene_col <- if ("gene" %in% colnames(sait_res_gene)) {
    "gene"
  } else if ("gene_name" %in% colnames(sait_res_gene)) {
    "gene_name"
  } else {
    "gene_id"
  }
...
test_that("plot_divergence_spectrum: p-value column detection in sait_res", {
  ...
  p_col <- if ("adj_p_interaction" %in% colnames(sait_res)) {
    "adj_p_interaction"
...
test_that("plot_divergence_spectrum: single gene from sait_res", {
  ...
  n_genes <- 3
  top_genes <- head(sait_res$gene, n_genes)
...
test_that("plot_sait: function call returns ggplot", {
  ...
  # Pass sait_res as list with results and model_data
  sait_res_list <- list(results = sait_res, model_data = model_data)
  
  # Function may warn with small datasets - accept any warning or no warning
  result <- suppressWarnings(TSENAT:::.plot_sait(
    se, sait_res_list,
    condition_col = "condition",
    n_top = 3,
    sig_alpha = 0.05
...
# Calculate SAIT interaction results for plotting tests
...
analysis_sait_jis_q12 <- suppressWarnings(calculate_jis(
    analysis,
    q = c(0.5, 1.0),
...
analysis_sait_jis_q08 <- suppressWarnings(calculate_jis(
    analysis,
    q = c(0.8),
...
# =============================================================================
# Test: plot_sait - Enhanced Assertions (CONSOLIDATED: 5 → 2 focused tests)
# =============================================================================
# OPTIMIZATION: Removed 3 nearly identical tests that all call plot_sait
...
test_that("plot_sait handles edge cases with valid output structures", {
    ...
    p_min <- plot_sait(analysis, n_top = 1)
...
test_that("Lazy switching_tables via results() produces valid output with different q-values", {
    ...
    tables_q12 <- results(analysis_sait_jis_q12, type = "switching_tables")
    ...
    tables_q08 <- results(analysis_sait_jis_q08, type = "switching_tables")
    expect_true(is.data.frame(tables_q08) || is.list(tables_q08))
    # Should not error even with minimal q values
    expect_silent({
        invisible(results(analysis_sait_jis_q08, type = "switching_tables"))
    })
})
...
test_that("Lazy switching_tables returns sorted/ordered output", {
    ...
    tables2 <- results(analysis_sait_jis_q12, type = "switching_tables")
...
test_that("SAIT results integrate properly with visualization pipeline", {
    ...
    expect_true(("gene" %in% colnames(sait_res)) || ("Gene" %in% colnames(sait_res)))
})
...
test_that("Lazy switching_tables handles edge cases (minimal/high q values)", {
    ...
    expect_silent({
        ...
        tables <- results(analysis_sait_jis_q08, type = "switching_tables")
    })
