...
#' `.calculate_sait()` results. Useful for understanding which
#' q-ranges (rare
...
#' @param sait_res A `data.frame` from `.calculate_sait()` with columns
...
#' 0.05).
#' Only used if genes = NULL; filters sait_res to significant genes before
#' selecting top n.
#' @param assay_name Name of the assay in `se` to extract (default:
#' 'diversity').
#' @param model_data Required list from `.calculate_sait(...,
...
#'
#' By providing `model_data` from `.calculate_sait()`, the
...
#' sait_result <- .calculate_sait(se, condition_col = 'condition',
#' method = 'gam',
#'                                       return_model_data = TRUE)
#' 
#' # Plot GAM curves for top genes
#' if (nrow(sait_result$results) > 0) {
#' grid_plot <- .plot_sait(se, sait_result$results,
#' condition_col = 'condition',
#'                                         n_top = 2,
#'  model_data = sait_result$model_data)
#' }
#'
...
.plot_sait <- function(se, sait_res, condition_col = "condition", genes = NULL, n_top = 6,
    ...
    validated <- .plot_gam_handle_inputs(se, sait_res)
    se <- validated$se
    sait_res <- validated$sait_res
    ...
    gene_name_map <- .plot_gam_create_gene_map(sait_res)
