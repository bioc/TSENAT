...
calculate_sait <- function(analysis, fdr_threshold = NULL, formula = NULL, condition_col = NULL,
...
# Helper: Extract and resolve SAIT interaction parameters from config @param
# analysis TSENATAnalysis object
.extract_sait_params <- function(analysis, condition_col = NULL, method = NULL, subject_col = NULL,
...
.combine_diversity_results_for_sait <- function(diversity_results) {
    tryCatch({
        assay_list <- list()
        coldata_list <- list()
        rowdata_first <- NULL
        ...
        for (key in q_keys) {
            ...
            if (!is(se, "SummarizedExperiment")) {
                stop("[combine_diversity_results_for_sait] Result ", key, " is not a SummarizedExperiment",
                  call. = FALSE)
            }
            ...
            if (length(SummarizedExperiment::assays(se)) == 0) {
                stop("[combine_diversity_results_for_sait] SE ", key, " has no assays",
                  call. = FALSE)
            }
        ...
        stop("[combine_diversity_results_for_sait] Failed: ", conditionMessage(e),
            call. = FALSE)
    })
}


# Helper: Build argument list for calculate_sait_interaction
.build_sait_args <- function(diversity_se, params, return_model_data = TRUE, verbose = FALSE,
    ...) {
...
# Helper: Validate and extract SAIT results from raw output
.validate_and_extract_sait_result <- function(result) {
    sait_results_df <- result
    model_data <- NULL

    # Extract components if result is list with $results and $model_data
    if (is.list(result) && "results" %in% names(result)) {
        sait_results_df <- result$results
        model_data <- result$model_data
    }

    # Validate structure
    if (!is.data.frame(sait_results_df)) {
        stop("[validate_and_extract_sait_result] Result must be data.frame, got: ",
            class(sait_results_df), call. = FALSE)
    }

    # Check for empty results
    if (nrow(sait_results_df) == 0 || ncol(sait_results_df) == 0) {
        warning("[validate_and_extract_sait_result] Result is empty (", nrow(sait_results_df),
            " rows, ", ncol(sait_results_df), " columns). ", "This can occur with: low sample counts per condition, ",
    ...
    missing_cols <- setdiff(required_cols, colnames(sait_results_df))
    if (length(missing_cols) > 0) {
        stop("[validate_and_extract_sait_result] Missing columns: ", paste(missing_cols,
            collapse = ", "), ". Available: ", paste(colnames(sait_results_df), collapse = ", "),
            call. = FALSE)
    }

    list(results = sait_results_df, model_data = model_data)
}


# Helper: Store SAIT results in analysis object
.store_sait_results_in_analysis <- function(analysis, sait_results_df, model_data = NULL) {
    if (is.list(analysis@sait_results) && "sait_interaction" %in% names(analysis@sait_results)) {
        analysis@sait_results$sait_interaction <- sait_results_df
        if (!is.null(model_data)) {
            analysis@sait_results$sait_interaction_model_data <- model_data
        }
    } else {
        analysis@sait_results <- list(sait_interaction = sait_results_df)
        if (!is.null(model_data)) {
            analysis@sait_results$sait_interaction_model_data <- model_data
        }
    }
    ...
    # Track function call
    analysis@metadata$function_calls <- c(analysis@metadata$function_calls, "calculate_sait")
