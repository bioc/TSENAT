...
#'   p_sait = runif(50, 0, 0.5),
#'   p_rank = runif(50, 0, 0.5),
#' agreement = sample(c('Both significant', 'SAIT only', 'Rank test only',
#' 'Neither significant'),
...
.plot_concordance <- function(comparison_df) {
    ...
    required_cols <- c("p_sait", "p_rank", "agreement")
    ...
    p1 <- ggplot2::ggplot(comparison_df, ggplot2::aes(x = -log10(.data$p_sait), y = -log10(.data$p_rank),
        ...
        ggplot2::scale_color_manual(values = c(`Both significant` = "#2ecc71", `SAIT only` = "#3498db",
    ...
    p_long <- data.frame(p_value = c(comparison_df$p_sait, comparison_df$p_rank), method = c(rep("SAIT",
...
.calculate_concordance <- function(analysis_sait, analysis_rank, sait_method = NULL,
    rank_method = NULL) {
    ...
    # If sait_method not specified, use the first available method
    if (is.null(sait_method)) {
        sait_method <- names(analysis_sait@sait_results)[1]
    }

    if (!(sait_method %in% names(analysis_sait@sait_results))) {
        available_methods <- paste(names(analysis_sait@sait_results), collapse = ", ")
        stop("SAIT method '", sait_method, "' not found. Available: ", available_methods,
            call. = FALSE)
    }

    gam_results <- analysis_sait@sait_results[[sait_method]]

    # Extract rank test results
    if (is.null(analysis_rank@rank_test_results) || length(analysis_rank@rank_test_results) ==
        0) {
        stop("No rank test results found in analysis_rank@rank_test_results. Run calculate_rank_transform() first.",
            call. = FALSE)
    }
    ...
    if (!is.data.frame(gam_results)) {
        stop("SAIT results ('", sait_method, "') must be a data.frame", call. = FALSE)
    }
    ...
    sait_p_col <- if ("p_interaction" %in% colnames(gam_results)) {
        ...
        stop("SAIT results missing required p-value column (p_interaction or p_value)",
            call. = FALSE)
    }

    sait_padj_col <- if ("adj_p_interaction" %in% colnames(gam_results)) {
        "adj_p_interaction"
    ...
    gam_genes <- gam_results$gene[!is.na(gam_results[[sait_p_col]])]
    ...
    if (length(common_genes) > 2) {
        ...
        comparison_df <- data.frame(gene = common_genes, p_sait = gam_results[[sait_p_col]][gam_idx],
            padj_sait = if (!is.na(sait_padj_col)) {
                gam_results[[sait_padj_col]][gam_idx]
            } else {
                rep(NA_real_, length(gam_idx))
            }, effect_sait = if ("effect_size" %in% colnames(gam_results)) {
        ...
        # 0.05)
        comparison_df$sait_sig <- comparison_df$padj_sait < 0.05
        comparison_df$rank_sig <- comparison_df$padj_rank < 0.05

        comparison_df$agreement <- ifelse(comparison_df$sait_sig & comparison_df$rank_sig,
            "Both significant", ifelse(comparison_df$sait_sig & !comparison_df$rank_sig,
                "SAIT only", ifelse(!comparison_df$sait_sig & comparison_df$rank_sig,
        ...
        high_conf <- comparison_df[comparison_df$sait_sig & comparison_df$rank_sig,
            ]

        # Sort by minimum p-value across methods
        if (nrow(high_conf) > 0) {
            high_conf <- high_conf[order(pmax(high_conf$p_sait, high_conf$p_rank)),
                ]
        }
    }
