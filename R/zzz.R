...
.onLoad <- function(libname, pkgname) {
    # Initialize lazy-loading flag for visualization dependencies See
    # R/lazy_load.R for lazy-loading implementation details
    assign(".viz_loaded", FALSE, envir = asNamespace(pkgname))

    # Ensure S3 method registration for print methods This makes sure R
    # recognizes our custom print methods
    ...
    # These warnings come from pretty() with degenerate axis ranges during
    # vignette rendering
        ...
        tryCatch({
            ...
            if (!is.null(.ggplot_print_original)) {
                registerS3method("print", "ggplot", function(x, ...) {
                  # Use withCallingHandlers to intercept and filter specific
                  # warnings
                  withCallingHandlers(.ggplot_print_original(x, ...), warning = function(w) {
                    # Suppress warnings from base R's pretty() when it
                    # encounters degenerate axis ranges (e.g., min() on empty
