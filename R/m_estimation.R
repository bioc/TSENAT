...
.huber_proposal2_scale <- function(y) {
    ...
    # scale k that balances efficiency and robustness
    ...
    for (iter in seq_len(10)) {
        ...
        # Check convergence
        if (abs(k_new - k0)/(k0 + 1e-06) < 1e-04)
            break
        k0 <- k_new
    }

    return(pmax(k0, 1e-06))
}

# Helper function: S-estimator scale
.mest_s_estimator_scale <- function(y, b = 0.5, max_iter = 20) {
    # S-estimator: minimizes scale with high breakdown point Target:
    # E[rho(y/s)] = b (typically b=0.5)
...
.mest_irls_location <- function(y, loss_type = c("huber", "tukey", "lsq"), scale = NULL, scale_method = c("mad", "proposal2", "s-estimator"),
    ...
    if (n_obs < 1) {
        return(if (return_weights) list(location_diff = NA, weights = numeric(0),
            scale_used = NA) else NA)
    }

    # Handle all-NA case
    if (all(is.na(y))) {
        return(if (return_weights) list(location_diff = NA, weights = rep(NA, n_obs),
            scale_used = NA) else NA)
    }
    ...
    if (is.null(scale)) {
        if (scale_method == "proposal2") {
            ...
            scale_local <- .mest_s_estimator_scale(y)
        } else {
            # Default to MAD
    ...
    for (iter in seq_len(max_iter)) {
        ...
        if (sum(weights[valid_idx]) > 0) {
            location <- sum(weights[valid_idx] * y[valid_idx], na.rm = TRUE)/sum(weights[valid_idx],
                na.rm = TRUE)
        } else {
        ...
        if (iter > 1) {
            ...
            if (!is.na(diff_current) && !is.infinite(diff_current) && diff_current <
                tol) {
                break
            }
        }
...
#' @noRd
.mest_prepare_se_data <- function(x, samples_col, q_combine_method = "mean") {
    ...
    group_assignment <- tryCatch({
        ...
        if (is.atomic(col_val))
            col_val else as.character(col_val)
    })
    ...
    for (j in seq_along(unique_samples)) {
        ...
        if (q_combine_method == "median") {
            entropy_by_sample[, j] <- apply(entropy_matrix[, cols_for_sample, drop = FALSE],
                1, median)
        } else {
            entropy_by_sample[, j] <- rowMeans(entropy_matrix[, cols_for_sample,
                drop = FALSE])
        }
    }
    ...
    list(entropy_by_sample = entropy_by_sample, group_assignment_unique = group_assignment_unique,
        unique_samples = unique_samples, sample_info = sample_info, sample_names_full = sample_names_full,
...
.mest_influence_loo <- function(entropy_by_sample, group_assignment_unique, unique_samples,
    ...
    for (i in seq_along(unique_samples)) {
        ...
        gene_level_changes[[unique_samples[i]]] <- data.frame(gene = gene_names,
            full_location_diff = m_est_full$location_diff, loo_location_diff = m_est_subset$location_diff,
            dfbeta = dfbeta, exceeds_threshold = abs(dfbeta) > dfbeta_threshold)
...
#' @noRd
.mest_compute_distances <- function(entropy_by_sample, group_assignment_unique, unique_samples) {
    ...
    for (group in unique(group_assignment_unique)) {
        ...
        if (length(group_samples_idx) > 0) {
            if (length(group_samples_idx) == 1) {
                ...
                group_centroid <- apply(entropy_by_sample[, group_samples_idx, drop = FALSE],
                  1, median)
            }
...
.handleMEstimateSEInput <- function(x, samples, q_combine_method, paired, scale,
    loss_type, max_iter, tol, pcorr, scale_method, influence_threshold, verbose) {
    # Helper: Process SummarizedExperiment input for m_estimate Returns list
    # with result_df and metadata for SE input

    se_data <- .mest_prepare_se_data(x, samples, q_combine_method)
    ...
    loo_result <- .mest_influence_loo(entropy_by_sample, group_assignment_unique,
        unique_samples, m_est_full, loss_type = loss_type, scale = scale, max_iter = max_iter,
        tol = tol, pcorr = pcorr, scale_method = scale_method)
    ...
    if (is.data.frame(m_est_full) && "max_weight" %in% colnames(m_est_full)) {
        ...
        sample_robustness_weights[] <- 1
    }

    # Calculate centroid distances
    centroid_distances <- .mest_compute_distances(entropy_by_sample, group_assignment_unique,
        unique_samples)
    ...
    result_df <- data.frame(Sample = names(sample_influence), Condition = condition_for_samples,
        Proportion_Affected = sample_influence, Genes_Affected = sample_influence *
            nrow(entropy_by_sample), Robustness_Weight = sample_robustness_weights,
        Entropy_Mean = apply(entropy_by_sample, 2, mean, na.rm = TRUE), Entropy_SD = apply(entropy_by_sample,
            2, sd, na.rm = TRUE), Distance_from_Centroid = centroid_distances, Status = ifelse(sample_influence >
            high_influence_threshold, "Flag for QC", "OK"), stringsAsFactors = FALSE,
        row.names = NULL)
...
.determineMEstimateScale <- function(y, scale = NULL, scale_method = "mad") {
    # Helper: Determine scale parameter for M-estimation Returns: scalar scale
    # value
...
#' @noRd
.performIRLSRegression <- function(y, X, loss_type = "huber", scale_local = 1, max_iter = 50,
    tol = 1e-06, use_intercept = TRUE) {
    ...
    for (iter in seq_len(max_iter)) {
        ...
        if (all(is.na(residuals))) {
            return(list(coef = if (use_intercept) c(NA_real_, NA_real_) else NA_real_,
                weights = rep(NA_real_, n_obs), converged = FALSE))
        }
...
.processMEstimateFeature <- function(feature_idx, x, samples, loss_type, scale, max_iter,
    ...
    # Helper: Process a single feature for M-estimation Returns: single-row
    ...
    irls_result <- .performIRLSRegression(y, X, loss_type = loss_type, scale_local = scale_local,
