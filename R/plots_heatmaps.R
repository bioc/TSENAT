...
#'   When \code{sait_results} is provided,  selects the n genes with 
#' lowest p-values.
#'   Otherwise, selects the first n genes from results.
#' @param sait_results Optional data.
#' frame from \code{\link{calculate_sait_interaction}()}
...
.plot_jis_delta <- function(switching_results, n_genes = 4, sait_results = NULL, verbose = FALSE,
    ...
    top_genes <- .heatmap_select_genes_multiq(switching_results, n_genes, sait_results)
...
#'   Supported sources:
#' - `.calculate_sait(..., return_model_data = TRUE)` returns a
...
#'
#' Ranks genes using SAIT results (if provided) and selects top N genes
...
#' @param sait_results Optional data.frame with SAIT interaction results
#'
#' @return Character vector of selected gene IDs
#'

#' @noRd
.heatmap_select_genes_multiq <- function(switching_results, n_genes = 4, sait_results = NULL) {
    ...
    # Find p-value column
    p_col <- if ("adj_p_interaction" %in% colnames(sait_results)) {
        "adj_p_interaction"
    } else if ("p_interaction" %in% colnames(sait_results)) {
        "p_interaction"
    } else {
        NULL
    }
    ...
    # Find gene identifier column in sait_results
    sait_gene_col <- if ("gene_id" %in% colnames(sait_results)) {
        "gene_id"
    } else if ("gene" %in% colnames(sait_results)) {
        "gene"
    } else if ("gene_name" %in% colnames(sait_results)) {
        "gene_name"
    } else {
        NULL
    }

    if (is.null(sait_gene_col)) {
        return(gene_ids[seq_len(min(n_genes, length(gene_ids)))])
    }

    # Rank genes by p-value (lowest = most significant)
    matches <- match(gene_ids, sait_results[[sait_gene_col]])
    p_values <- rep(Inf, length(gene_ids))
    matched_idx <- !is.na(matches)
    p_values[matched_idx] <- sait_results[[p_col]][matches[matched_idx]]
...
#' calculate_sait_interaction or similar statistical results), and returns
