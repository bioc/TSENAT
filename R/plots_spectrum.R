...
#' @param sait_res Optional data.frame with columns for gene identifiers and
#' p-values. Used to select
#'   top genes when gene = NULL and sait_res is provided. Default: NULL.
...
.spectrum_find_gene_column <- function(df) {
    ...
    if (length(found) == 0) {
        stop("'sait_res' must have a column named 'gene', 'gene_name', or 'gene_id'",
            call. = FALSE)
    }
    return(found[1])
}
...
.spectrum_find_pvalue_column <- function(df) {
    ...
    found <- valid_cols[valid_cols %in% colnames(df)]
    if (length(found) == 0) {
        stop("'sait_res' must have a p-value column like adj_p_interaction or p_value",
            call. = FALSE)
    }
    return(found[1])
}
...
#' @noRd
.spectrum_validate_inputs <- function(divergence_results_se, gene, sait_res, n_genes,
    ncol) {
    ...
    if (!is.null(sait_res) && (!is.data.frame(sait_res) || nrow(sait_res) == 0)) {
        stop("'sait_res' must be a non-empty data.frame or NULL", call. = FALSE)
    }
...
#' @noRd
.spectrum_plot_top_genes <- function(sait_res, n_genes_use, ncol, div_mat_sorted, q_vals_sorted,
    gene_names, metric, divergence_results_se) {
    gene_col <- .spectrum_find_gene_column(sait_res)
    p_col <- .spectrum_find_pvalue_column(sait_res)
    sait_sorted <- sait_res[order(sait_res[[p_col]], na.last = TRUE), , drop = FALSE]
    top_genes_vec <- head(sait_sorted[[gene_col]], n_genes_use)

    if (length(top_genes_vec) == 0) {
        stop("No genes found in 'sait_res'", call. = FALSE)
    }
    ...
    plot_list <- lapply(seq_along(gene_indices), function(i) {
        df <- data.frame(q = q_vals_sorted, divergence = as.numeric(div_mat_sorted[gene_indices[i],
            ]), gene = gene_names[gene_indices[i]], p_value = sait_sorted[[p_col]][i],
            stringsAsFactors = FALSE)
...
.plot_divergence_spectrum <- function(divergence_results_se, gene = NULL, sait_res = NULL,
    ...
    div_mat <- .spectrum_validate_inputs(divergence_results_se, gene, sait_res, n_genes,
        ncol)
    ...
    if (!is.null(sait_res)) {
        return(.spectrum_plot_top_genes(sait_res, n_genes, ncol, div_mat_sorted, q_vals_sorted,
            gene_names, metric, divergence_results_se))
    }
