...
- **GAM / GAMM** (generalized additive models / mixed models): flexibly capture nonlinear q-response patterns via adaptive smoothing splines. For unpaired (cross-sectional) designs, GAM is used directly. When `paired = TRUE`, `method = "gam"` dispatches to a **spline-based GAMM-style path implemented with `nlme::lme`** (natural regression splines, `ns(q, df = 3) × condition`), subject-level random intercepts and an AR(1) working correlation within each subject × condition block (marginal F-test for the interaction). `mgcv::gamm()` and standard GAM serve as fallbacks. GAMM can also be selected explicitly with `method = "gamm"`.
...
- **Shape**: As defined in the pattern types above, a flat curve (balanced) vs. declining curve (rare driven) vs. rising curve (abundant driven) encodes fundamentally different mechanisms [@I059]; the full curve is the complete story, not any single q point [@I021].
