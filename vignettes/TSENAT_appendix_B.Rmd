---
title: "Appendix B: Non-Parametric Validation of Scale-Adaptive Interaction Test Results via GAMM and Aligned Rank Transform (ART)"
...
**Method 2: Aligned Rank Transform (ART)** — State-of-the-art non-parametric interaction testing, with Hochberg Step-Up Correction. Falls back to Conover-Iman Rank Transform via `method='rt'` if needed.
...
- Treatment of q-ordering: Two-way ART on raw entropy values, treating q-values as ordered factors; aligned ranks decomposed via ANOVA with proper F-test for interactions.
...
| Interaction testing | Standard ANOVA-type | Proper interaction tests via alignment [@S271] |
...
ART handles this structure through its alignment step: within-subject ranking after stripping main effects preserves the pairing structure while removing the influence of AR(1) dependence [@S271; @S272]. Autocorrelation in the original entropy values does NOT affect aligned rank ordering or the validity of inference — this property, known as the *exchangeability property* of rank-based tests, ensures that rank-based inference is valid under any correlation pattern [@S019; @C018].
